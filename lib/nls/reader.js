...
function readDirectory(dir) {
  return fs.readdirAsync(dir).then(function(paths) {
    var i, len = paths.length,
      results = new Array(len);
    for (i = 0; i < len; ++i) {
      results.push(readStats(path.join(dir, paths[i])));
    }
    return Bluebird.all(results).then(function(paths) {
...
NLSReader.prototype.readBundleContents = function(nls) {
  var i, len = nls.length,
    bundles = new Array(len);
  for (i = 0; i < len; ++i) {
...
NLSReader.prototype.createBundle = function(nls) {
  ...
  for (lang in nls.lang) {
    for (key in nls.lang[lang]) {
      if (!strings[key]) {
        strings[key] = new LocalizedString({
          id: key
        });
      }
