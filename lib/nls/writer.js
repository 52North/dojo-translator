...
var mkdir = Promise.denodeify(require('mkdirp'));
...
NLSWriter.prototype.getFile = function(target, bundle, lang) {
  var relPath = bundle.getId().replace(/\./g, path.sep);
  var basePath = path.join(target, 'bundles', relPath, 'nls');
  if (lang) {
...
NLSWriter.prototype.writeLang = function(target, bundle, lang) {
  var file = this.getFile(target, bundle, lang);
  var content = this.bundleToString(bundle, lang);
  return mkdir(path.dirname(file)).then(function() {
    return fs.riteFile(file, content);
  });
};

NLSWriter.prototype.writeBundle = function(target, bundle) {
  var lang = bundle.getLanguages();
  var i, len = lang.length, p = new Array(len);
  this.writeLang(target, bundle);
  for (i = 0; i < len; ++i) {
    if (lang[i] !== this.defaultLang) {
      p[i] = this.writeLang(target, bundle, lang[i]);
    }
  }
