...
PostProcessor.prototype.correctPlaceHolders = function(x) {
  ...
  // {} name $ > ${name}
  x.translation = x.translation.replace(/\{\} ([a-zA-Z]+) \$/g, replacer);
  // dollarit {} itemcount >  ${itemCount}
  x.translation = x.translation.replace(/ dollarit \{\} itemcount/ig, "/${itemCount}");
  // dollarit {} endPage >  ${endPage}
  x.translation = x.translation.replace(/ dollarit \{\} endpage/ig, "/${endPage}");
  // Резултати pageStartItemNumber} ${-} ${pageEndItemNumber на ${itemCount}
  x.translation = x.translation.replace(/ pageStartItemNumer\} \$\{-\} \{pageEndItemNumber[ /]/ig,
                                        "${pageStartItemNumer\} - ${pageEndItemNumber} ");
};
...
PostProcessor.prototype.checkPlaceHolders = function(re, x) {
