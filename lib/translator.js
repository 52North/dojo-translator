...
Translator.prototype.queueWorker = function(task, callback) {
  ...
  var qs = { key: this.key, source: slang, target: tlang, q: ss };
  ...
  if (this.noop) {
    return Promise.delay(50).done(function() {
      s.addTranslation(tlang, ss);
      return callback();
    });
  }

  this.request({ qs: qs }, function(err, response, body) {
...
Translator.prototype.translate = function(bundles, lang) {
  ...
  return Promise.all(jobs).then(function(){ return bundles; });
};
